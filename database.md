...
## Narrative
  ...
  The program is a simple but fun game that Pokémon fans can play, and the game utilizes the many programming skills that I have learned over the years. It utilizes a GUI with Windows Forms and the .NET framework, and originally it used external files to read in an manipulate the data, which is a skill useful for most programming projects. It also showcases my ability to plan, organize, develop, and maintain a project.
  
  My goal for this enhancement was to transfer the Pokemon information used in the project from an XML file to a MongoDB collection, which would allow for the information to be maintained easier while also giving me experience connecting to and retrieving information from a MongoDB. Once the collection was created and the information was transferred, I updated the algorithms within the program to utilize the MongoDB rather than the XML file. In addition to this, I created a python script that utilized a RESTful API using the Bottle framework. Users can connect to the API, which can filter the list of possible Pokémon that the game has currently selected based on the hints that the game provides to the player after each guess. Using query strings in the URL path, a list of possible Pokémon would be returned.
